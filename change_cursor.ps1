...
$cursorsKey = "HKEY_CURRENT_USER\Control Panel\Cursors"
...
"
   ...
   Switch ($userInput) {
		...
		'o' {opencursorSettings; Break}
		4 {opencursorSettings; Break}
		'e' {exit}
		"exit" {exit}
		5 {exit}
...
function createScheme {
	...
	if ($cursorPath.Length -eq 0) { $cursorPath = Get-Location } 
	while (-Not (Test-Path -Path $cursorPath -PathType Container)) {
		Write-Host -ForegroundColor "Red" "directiory does not exist!"
		$cursorPath = Read-Host -Prompt "directiory: "
	}
	applycursor -folderPath "$cursorPath" -schemeName "$schemeName"
}
...
function applycursor {	
	...
	foreach ($searchWord in $arrow_search_strings) {
		...
		if ($matchingFiles.Count -gt 0) {
			...
			New-ItemProperty -Path "Registry::$cursorsKey" -Name "Arrow" -Value "$arrow" -Force | Out-Null
			break
		}
	}
	...
	foreach ($searchWord in $helpsel_search_Strings) {
		...
		if ($matchingFiles.Count -gt 0) {
			...
			New-ItemProperty -Path "Registry::$cursorsKey" -Name "Help" -Value "$helpsel" -Force | Out-Null
			break
		}
	}
	...
	foreach ($searchWord in $working_search_Strings) {
		...
		if ($matchingFiles.Count -gt 0) {
			...
			New-ItemProperty -Path "Registry::$cursorsKey" -Name "AppStarting" -Value "$working" -Force | Out-Null
			break
		}
	}
	...
	foreach ($searchWord in $busy_search_Strings) {
		...
		if ($matchingFiles.Count -gt 0) {
			...
			New-ItemProperty -Path "Registry::$cursorsKey" -Name "Wait" -Value "$busy" -Force | Out-Null
			break
		}
	}
	...
	foreach ($searchWord in $precisionsel_search_Strings) {
		...
		if ($matchingFiles.Count -gt 0) {
			...
			New-ItemProperty -Path "Registry::$cursorsKey" -Name "Crosshair" -Value "$precisionsel" -Force | Out-Null
			break
		}
	}
	...
	foreach ($searchWord in $txtsel_search_Strings) {
		...
		if ($matchingFiles.Count -gt 0) {
			...
			New-ItemProperty -Path "Registry::$cursorsKey" -Name "IBeam" -Value "$txtsel" -Force | Out-Null
			break
		}
	}
	...
	foreach ($searchWord in $pen_search_Strings) {
		...
		if ($matchingFiles.Count -gt 0) {
			...
			New-ItemProperty -Path "Registry::$cursorsKey" -Name "NWPen" -Value "$pen" -Force | Out-Null
			break
		}
	}
	...
	foreach ($searchWord in $unavail_search_Strings) {
		...
		if ($matchingFiles.Count -gt 0) {
			...
			New-ItemProperty -Path "Registry::$cursorsKey" -Name "No" -Value "$unavail" -Force | Out-Null
			break
		}
	}
	...
	foreach ($searchWord in $ns_search_Strings) {
		...
		if ($matchingFiles.Count -gt 0) {
			...
			New-ItemProperty -Path "Registry::$cursorsKey" -Name "SizeNS" -Value "$ns" -Force | Out-Null
			break
		}
	}
	...
	foreach ($searchWord in $ew_search_Strings) {
		...
		if ($matchingFiles.Count -gt 0) {
			...
			New-ItemProperty -Path "Registry::$cursorsKey" -Name "SizeWE" -Value "$ew" -Force | Out-Null
			break
		}
	}
	...
	foreach ($searchWord in $nwse_search_Strings) {
		...
		if ($matchingFiles.Count -gt 0) {
			...
			New-ItemProperty -Path "Registry::$cursorsKey" -Name "SizeNWSE" -Value "$nwse" -Force | Out-Null
			break
		}
	}
	...
	foreach ($searchWord in $nesw_search_Strings) {
		...
		if ($matchingFiles.Count -gt 0) {
			...
			New-ItemProperty -Path "Registry::$cursorsKey" -Name "SizeNESW" -Value "$nesw" -Force | Out-Null
			break
		}
	}
	...
	foreach ($searchWord in $move_search_Strings) {
		...
		if ($matchingFiles.Count -gt 0) {
			...
			New-ItemProperty -Path "Registry::$cursorsKey" -Name "SizeAll" -Value "$move" -Force | Out-Null
			break
		}
	}
	...
	foreach ($searchWord in $up_search_Strings) {
		...
		if ($matchingFiles.Count -gt 0) {
			...
			New-ItemProperty -Path "Registry::$cursorsKey" -Name "UpArrow" -Value "$up" -Force | Out-Null
			break
		}
	}
	...
	foreach ($searchWord in $link_search_Strings) {
		...
		if ($matchingFiles.Count -gt 0) {
			...
			New-ItemProperty -Path "Registry::$cursorsKey" -Name "Hand" -Value "$link" -Force | Out-Null
			break
		}
	}
	...
	foreach ($searchWord in $pin_search_Strings) {
		...
		if ($matchingFiles.Count -gt 0) {
			...
			New-ItemProperty -Path "Registry::$cursorsKey" -Name "Pin" -Value "$pin" -Force | Out-Null
			break
		}
	}
	...
	foreach ($searchWord in $person_search_Strings) {	
		...
		if ($matchingFiles.Count -gt 0) {
			...
			New-ItemProperty -Path "Registry::$cursorsKey" -Name "Person" -Value "$person" -Force | Out-Null
			break
		}
	}
